...
class Player2(pygame.sprite.Sprite):
    ...
    GRAVITY = 1
    animation_delay = 3
    def __init__(self,x,y,width,height,name,character_name,Fruit_name,lives):
        super().__init__()
        ...
        self.SPRITES = load_sprite_sheet("MainCharacters",self.character_name, 32, 32, True)
        self.rect_old = self.rect.copy()  # Khởi tạo tọa độ cũ
        self.is_double_jumping = False
        self.double_jump = "abc"
        self.opponent_fruits = []
        self.lives = lives
    ...
    def update(self):
        self.rect_old = self.rect.copy()  # Cập nhật tọa độ cũ trước khi cập nhật tọa độ mới
    ...
    def update_sprite(self):
        ...
        if self.x_vel != 0:
            sprite_sheet = "run"
            self.double_jump ="abc"
        elif self.rect.x < self.rect_old.x:
            self.direction = "left"
        elif self.rect.x > self.rect_old.x:
            self.direction = "right"
        if self.double_jump =="double_jump":
        ...
        self.update()
        # Cập nhật vị trí cũ của nhân vật
